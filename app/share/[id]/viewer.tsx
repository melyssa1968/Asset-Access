...
export default function ShareViewer({id}:{id:string}){
 ...
 useEffect(()=>{fetch(`${BASE_PATH}/api/public/share/${id}`).then(async r=>{const j=await r.json();if(!r.ok)throw new Error(j.error);setMeta(j);setLoading(false);if(!j.link.requireEmail)await grant("")}).catch(e=>{setError(e.message);setLoading(false)})},[id]);
 useEffect(()=>{if(!granted)return;const send=()=>{const durationMs=Date.now()-started.current;fetch(`${BASE_PATH}/api/public/share/${id}/track`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({event:"heartbeat",durationMs,progress:Math.min(95,Math.round(durationMs/3000))}),keepalive:true}).catch(()=>{})};const timer=setInterval(send,15000);return()=>{clearInterval(timer);send()}},[granted,id]);
 async function grant(value:string){const r=await fetch(`${BASE_PATH}/api/public/share/${id}/access`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({email:value})}),j=await r.json();if(!r.ok){setError(j.error);return}setGranted(true);setError("")}
 async function submit(e:FormEvent){e.preventDefault();await grant(email)}
 function download(){fetch(`${BASE_PATH}/api/public/share/${id}/track`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({event:"download",durationMs:Date.now()-started.current,progress:100}),keepalive:true}).catch(()=>{})}
 ...
 const pdf=meta.asset.contentType==="application/pdf",video=meta.asset.contentType.startsWith("video/"),fileUrl=`${BASE_PATH}/api/public/share/${id}/file`;

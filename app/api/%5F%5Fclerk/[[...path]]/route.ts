...
async function proxyToClerk(request: Request) {
  ...
  const internalPath = source.pathname
    .replace(/^\/asset-access/, "")
    .replace(/^\/api\/__clerk/, "");
  const target = new URL(internalPath || "/", CLERK_FRONTEND_API);
  ...
  if (location) {
    ...
    if (redirect.origin === CLERK_FRONTEND_API) {
      responseHeaders.set(
        "location",
        `${PUBLIC_PROXY_URL}/${redirect.pathname.replace(/^\//, "")}${redirect.search}`,
      );
    }
  }
